-- Reparo idempotente e autossuficiente para bancos que nao receberam
-- a tabela de anotacoes ou as colunas de documentos.
...
drop policy if exists "mestre gerencia anotacoes da campanha"
on public.anotacoes_campanha;
...
using (
  exists (
    ...
    where c.id = campanha_id and c.mestre_id = auth.uid()
  )
)
with check (
  exists (
