...
import { ativarCena, atualizarModoCampanha, buscarCampanhaPorCodigo, chavePosicaoMapa, desvincularFicha, desvincularInimigo, enviarImagemCena, excluirCena, moverToken, ouvirCampanha, posicionarFichaNoMapa, posicionarInimigoNoMapa, registrarRolagem, salvarCena, validarArquivoImagem, vincularFicha, vincularInimigo } from "../services/mesaApi";
import { buscarPersonagem, listarPersonagens } from "../services/personagemApi";
...
const Mesa = () => {
  ...
  const [bibliotecaAberta, setBibliotecaAberta] = useState(mestre);
  ...
  const [abaLateralMestre, setAbaLateralMestre] = useState("participantes");
  const [abaLateralJogador, setAbaLateralJogador] = useState("participantes");
  ...
  return <main className={`mesa-shell ${mestre ? "visao-mestre" : "visao-jogador"} ${!mestre && abaLateralJogador === "ficha" ? "jogador-ficha-ativa" : ""} modo-atual-${campanha.modo} ${bibliotecaAberta ? "com-biblioteca" : ""}`} style={{ "--mesa-acento": corCena || personagemJogador?.temaFicha?.primaria || "#d4af37", "--mesa-secundaria": corCena || personagemJogador?.temaFicha?.secundaria || "#8b4513", "--mesa-texto": personagemJogador?.temaFicha?.texto || "#fff8ea", "--mesa-fundo": personagemJogador?.temaFicha?.fundo || "#080808", "--mesa-borda": corCena ? `color-mix(in srgb,${corCena} 45%,transparent)` : (personagemJogador?.temaFicha?.borda || "rgba(212,175,55,.35)") }}>
    ...
    {mestre && musicaAberta && <aside className="mesa-musica-painel"><header><div><span>Trilha da sessao</span><strong>Musica do YouTube</strong></div><button onClick={() => setMusicaAberta(false)} title="Fechar musica"><Icon path={mdiClose} size={.85} /></button></header><MusicaTabletop campanhaId={campanha.id} /></aside>}
    ...
    {mestre && <nav className="mesa-lateral-navegacao" aria-label="Conteudo da lateral"><button className={abaLateralMestre === "participantes" ? "ativo" : ""} onClick={() => { setAbaLateralMestre("participantes"); setInimigoAberto(null); }}>Participantes <b>{campanha.membros.length}</b></button><button className={abaLateralMestre === "inimigos" ? "ativo" : ""} onClick={() => setAbaLateralMestre("inimigos")}>Inimigos <b>{campanha.inimigos?.length || 0}</b></button></nav>}
    {!mestre && <nav className="mesa-lateral-navegacao mesa-jogador-navegacao" aria-label="Conteudo do jogador"><button className={abaLateralJogador === "participantes" ? "ativo" : ""} onClick={() => setAbaLateralJogador("participantes")}>Participantes <b>{campanha.membros.length}</b></button><button className={abaLateralJogador === "ficha" ? "ativo" : ""} onClick={() => setAbaLateralJogador("ficha")}>Ficha</button></nav>}
    {!mestre && abaLateralJogador === "participantes" && <aside className="mesa-jogador-participantes"><div className="mesa-lateral-titulo"><Icon path={mdiShieldCrownOutline} size={0.8} /><span>Participantes</span><b>{campanha.membros.length}</b></div><div className="mesa-membros">{campanha.membros.length ? campanha.membros.map((membro) => { const personagem = personagens[membro.ficha_id]; const integridade = calcularIntegridade(personagem); const recursos = personagem ? [{ id: "sanidade", nome: "SAN", valor: personagem.sanidade }, { id: "esperanca", nome: "ESP", valor: personagem.esperanca }, { id: "integridade", nome: "INT", valor: integridade }] : []; return <div className={`mesa-jogador-participante ${membro.ficha_id === fichaJogadorId ? "eu" : ""}`} key={membro.id}><span>{personagem?.fotoPerfil ? <img src={personagem.fotoPerfil} alt="" /> : membro.nome?.slice(0, 2)}</span><div><strong>{personagem?.nome || membro.nome}</strong><small>{personagem ? `Nivel ${personagem.nivel || 1} · ${personagem.classe || "Sem classe"}` : membro.papel}</small>{recursos.length > 0 && <div className="membro-recursos">{recursos.map((recurso) => <div className={`membro-recurso ${recurso.id}`} key={recurso.id}><span>{recurso.nome}</span><i><em style={{ width: `${percentualRecurso(recurso.valor)}%` }} /></i><b>{recurso.valor?.atual || 0}/{recurso.valor?.max || 0}</b></div>)}</div>}</div></div>; }) : <p>Nenhuma ficha vinculada ainda.</p>}</div></aside>}
