...
const Mesa = () => {
  ...
  useEffect(() => {
    if (!mestre) return;
    try {
      const inimigos = (JSON.parse(localStorage.getItem("darkness_inimigos")) || []).map((item) => ({ ...item, tipo: "inimigo" }));
      const npcs = (JSON.parse(localStorage.getItem("darkness_npcs")) || []).map((item) => ({ ...item, tipo: "npc" }));
