...
const ControleVitalInimigo = ({ nome, recurso, aoAlterar, tipo = "" }) => {
  ...
  const percentual = max > 0 ? Math.max(0, Math.min(100, atual / max * 100)) : 0;
  ...
  const aplicarAjuste = (operacao) => {
    ...
    const resultado = operacao === "subtrair" ? atual - quantidade : atual + quantidade;
    aoAlterar(Math.max(0, max ? Math.min(max, resultado) : resultado));
    setAjuste("");
  };
  return <div className={`inimigo-vital ${tipo} ${estado}`}><div><span>{nome}</span><b>{atual} / {max}</b></div><i><em style={{ width: `${percentual}%` }} /></i><div className="inimigo-vital-controles"><button type="button" onClick={() => aplicarAjuste("subtrair")} disabled={!Number(ajuste)} aria-label={`Subtrair de ${nome}`} title={`Subtrair de ${nome}`}>−</button><input type="number" min="0" inputMode="numeric" placeholder="Valor" value={ajuste} onChange={(event) => setAjuste(event.target.value.replace(/[^0-9]/g, ""))} aria-label={`Valor para alterar ${nome}`} /><button type="button" onClick={() => aplicarAjuste("somar")} disabled={!Number(ajuste)} aria-label={`Somar em ${nome}`} title={`Somar em ${nome}`}>+</button></div></div>;
};
...
const Mesa = () => {
  ...
  const salvarVitalInimigo = async (inimigoAtualizado) => {
    ...
    setCampanha((atual) => ({
      ...atual,
      inimigos: (atual.inimigos || []).map((item) => item.id === inimigoAtualizado.id ? inimigoAtualizado : item),
    }));
    try {
      const salvo = await atualizarInimigoCampanha(campanha.id, inimigoAtualizado);
      setInimigoAberto((atual) => atual?.id === salvo.id ? salvo : atual);
      setCampanha((atual) => ({
        ...atual,
        inimigos: (atual.inimigos || []).map((item) => item.id === salvo.id ? salvo : item),
      }));
    } catch (error) {
  ...
  const alterarSanidadeInimigo = (valor) => salvarVitalInimigo({
    ...inimigoAberto,
    sanidade: { ...(inimigoAberto.sanidade || {}), atual: valor },
  });
  const alterarMembroInimigo = (membro, valor) => salvarVitalInimigo({
    ...inimigoAberto,
    membros: {
  ...
  return <main className={`mesa-shell ${mestre ? "visao-mestre" : "visao-jogador"} ${!mestre && abaLateralJogador === "ficha" ? "jogador-ficha-ativa" : ""} ${!(mestre ? abaLateralMestre : abaLateralJogador) ? "painel-lateral-fechado" : ""} modo-atual-${campanha.modo} ${bibliotecaAberta ? "com-biblioteca" : ""}`} style={{ "--mesa-acento": corCena || personagemJogador?.temaFicha?.primaria || "#d4af37", "--mesa-secundaria": corCena || personagemJogador?.temaFicha?.secundaria || "#8b4513", "--mesa-texto": personagemJogador?.temaFicha?.texto || "#fff8ea", "--mesa-fundo": personagemJogador?.temaFicha?.fundo || "#080808", "--mesa-borda": corCena ? `color-mix(in srgb,${corCena} 45%,transparent)` : (personagemJogador?.temaFicha?.borda || "rgba(212,175,55,.35)") }}>
    ...
    <header className="mesa-topo">
      <a href={urlRetorno} title={mestre ? "Voltar ao dashboard" : "Voltar para a ficha"}><Icon path={mdiArrowLeft} size={0.9} /></a>
