...
import { mdiAccountMultipleOutline, mdiArrowLeft, mdiChevronDown, mdiChevronRight, mdiDeleteOutline, mdiFolderOutline, mdiImagePlusOutline, mdiMapOutline, mdiPencilOutline, mdiPlus } from "@mdi/js";
import { buscarCampanhaPorCodigo, criarCampanhaMesa, desvincularFicha, enviarImagemCena, excluirCampanhaMesa, excluirCena, listarCampanhas, salvarCena, validarArquivoImagem, vincularFicha } from "../services/mesaApi";
...
const CampanhaDashboard = () => {
  ...
  return <section className="campanha-dashboard">
    ...
    <nav className="campanha-conteudo-abas"><button className={abaCampanha === "cenas" ? "ativo" : ""} onClick={() => setAbaCampanha("cenas")}>Cenas e mapas</button><button className={abaCampanha === "anotacoes" ? "ativo" : ""} onClick={() => setAbaCampanha("anotacoes")}>Anotacoes do mestre</button></nav>
    {abaCampanha === "cenas" ? <div className="campanha-pastas">{grupos.length ? grupos.map(([pasta, cenas]) => <section className="campanha-pasta" key={pasta}><button className="campanha-pasta-titulo" onClick={() => setPastasFechadas((estado) => ({ ...estado, [pasta]: !estado[pasta] }))}><Icon path={pastasFechadas[pasta] ? mdiChevronRight : mdiChevronDown} size={.72} /><Icon path={mdiFolderOutline} size={.78} /><strong>{pasta}</strong><span>{cenas.length}</span></button>{!pastasFechadas[pasta] && <div className="campanha-cenas">{cenas.map((cena) => <article key={cena.id}><div className="campanha-cena-miniatura" style={{ backgroundImage: `url(${cena.imagensCena?.[0]?.url || cena.mapasBatalha?.[0]?.url || urlCena(cena, "cena") || urlCena(cena, "mapa")})` }}>{cena.mapasBatalha?.length > 0 && <span><Icon path={mdiMapOutline} size={.62} />{cena.mapasBatalha.length} mapa(s)</span>}</div><div className="campanha-cena-info"><strong>{cena.nome}</strong><p>{cena.descricao || "Sem descricao"}</p><small>{cena.imagensCena?.length || 0} cenas estaticas · {cena.mapasBatalha?.length || 0} mapas</small></div><div className="campanha-cena-acoes"><button onClick={() => abrirEditor(cena)} title="Editar"><Icon path={mdiPencilOutline} size={.72} /></button><button onClick={() => remover(cena)} title="Excluir"><Icon path={mdiDeleteOutline} size={.72} /></button></div></article>)}</div>}</section>) : <div className="campanha-sem-cenas"><Icon path={mdiImagePlusOutline} size={1.3} /><strong>Nenhuma cena salva</strong><button onClick={() => abrirEditor()}>Criar primeira cena</button></div>}</div> : <AnotacoesCampanha campanhaId={campanha.id} />}
