...
import { mdiAccountMultipleOutline, mdiArrowLeft, mdiChevronDown, mdiChevronRight, mdiDeleteOutline, mdiDrag, mdiFolderOutline, mdiImagePlusOutline, mdiMapOutline, mdiPlaylistMusic, mdiPencilOutline, mdiPlus } from "@mdi/js";
import { buscarCampanhaPorCodigo, criarCampanhaMesa, desvincularFicha, enviarImagemCena, excluirCampanhaMesa, excluirCena, listarCampanhas, ordenarCenasCampanha, salvarCena, salvarMusicasCampanha, salvarSoundpadCampanha, validarArquivoImagem, vincularFicha } from "../services/mesaApi";
...
const CampanhaDashboard = () => {
  ...
  useEffect(() => { setSonsSoundpad(campanha?.soundpad || []); }, [campanha?.soundpad]);
  ...
  const removerCampanha = async (item) => {
    if (!await confirmarDialogo(`Excluir a campanha "${item.nome}" e todas as suas cenas?`, { titulo: "Excluir campanha", confirmarTexto: "Excluir", perigo: true })) return;
  ...
  const grupos = useMemo(() => {
    ...
    return Object.entries(resultado).sort(([, cenasA], [, cenasB]) =>
      Math.min(...cenasA.map((cena) => Number(cena.ordem) || 0)) - Math.min(...cenasB.map((cena) => Number(cena.ordem) || 0))
    );
  }, [campanha?.cenas]);

  const pastas = useMemo(() => grupos.map(([nome]) => nome), [grupos]);
  const abrirEditor = (cena = null) => {
    setEditando(cena ? { ...cena, pasta: cena.pasta || "Sem pasta", imagensCena: cena.imagensCena || [], mapasBatalha: cena.mapasBatalha || [] } : { ...cenaVazia, ordem: campanha?.cenas?.length || 0 });
  ...
  const adicionarMidia = (tipo) => {
    ...
    const item = { id: `${tipo}-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`, nome: `${prefixo} ${(editando[chave] || []).length + 1}`, url: "", arquivo: null, ...(tipo === "mapa" ? { larguraGrade: 12, alturaGrade: 8, exibirGrade: true } : {}) };
  ...
  const remover = async (cena) => {
    if (!await confirmarDialogo(`Excluir "${cena.nome}"?`, { titulo: "Excluir cena", confirmarTexto: "Excluir", perigo: true })) return;
  ...
  const posicaoDrop = (event) => event.clientY >= event.currentTarget.getBoundingClientRect().top + event.currentTarget.getBoundingClientRect().height / 2 ? "abaixo" : "acima";
  const moverItem = (itens, origemId, destinoId, posicao = "acima") => {
    ...
    const destinoDepoisDaRemocao = novos.findIndex((item) => item.id === destinoId);
    novos.splice(destinoDepoisDaRemocao + (posicao === "abaixo" ? 1 : 0), 0, movido);
    return novos;
  };
  const soltarCena = async (destino) => {
    if (arrastando?.tipo !== "cena" || arrastando.id === destino.id || arrastando.pasta !== (destino.pasta || "Sem pasta")) { setArrastando(null); return; }
    const cenas = moverItem(campanha.cenas, arrastando.id, destino.id, indicadorDrop?.posicao).map((cena, ordem) => ({ ...cena, ordem }));
    setCampanha((atual) => ({ ...atual, cenas }));
    setArrastando(null); setIndicadorDrop(null);
    try { await ordenarCenasCampanha(campanha.id, cenas); }
    catch (error) { setErro(error.message || "Nao foi possivel salvar a ordem das cenas."); await recarregarCampanha(); }
  };
  const soltarMusica = (destinoId) => {
    if (arrastando?.tipo === "musica") setMusicas((atuais) => moverItem(atuais, arrastando.id, destinoId, indicadorDrop?.posicao));
    setArrastando(null); setIndicadorDrop(null);
  };
  const soltarPasta = async (pastaDestino) => {
    if (arrastando?.tipo !== "pasta" || arrastando.id === pastaDestino) { setArrastando(null); setIndicadorDrop(null); return; }
    const pastasOrdenadas = grupos.map(([pasta]) => ({ id: pasta }));
    const novaOrdemPastas = moverItem(pastasOrdenadas, arrastando.id, pastaDestino, indicadorDrop?.posicao).map((item) => item.id);
    const cenas = novaOrdemPastas.flatMap((pasta) => (campanha.cenas || []).filter((cena) => (String(cena.pasta || "Sem pasta").trim() || "Sem pasta") === pasta)).map((cena, ordem) => ({ ...cena, ordem }));
    setCampanha((atual) => ({ ...atual, cenas }));
    setArrastando(null); setIndicadorDrop(null);
    try { await ordenarCenasCampanha(campanha.id, cenas); }
    catch (error) { setErro(error.message || "Nao foi possivel salvar a ordem das pastas."); await recarregarCampanha(); }
  };
  ...
  const adicionarSom = () => setSonsSoundpad((atuais) => [...atuais, { id: `som-${Date.now()}`, nome: `Efeito ${atuais.length + 1}`, categoria: "Efeito", url: "", volume: 70, loop: false, sobrepor: true, fadeIn: .4, fadeOut: .7 }]);
  const atualizarSom = (id, mudanca) => setSonsSoundpad((atuais) => atuais.map((som) => som.id === id ? { ...som, ...mudanca } : som));
  const removerSom = (id) => setSonsSoundpad((atuais) => atuais.filter((som) => som.id !== id));
  const selecionarAudioSom = (id, arquivo, input) => {
    try { validarArquivoImagem(arquivo, "audio"); setErro(""); atualizarSom(id, { arquivo, arquivoNome: arquivo.name }); }
    catch (error) { setErro(error.message); if (input) input.value = ""; }
  };
  const confirmarSoundpad = async (event) => {
    event.preventDefault(); setSalvando(true); setErro("");
    try {
      const sons = await Promise.all(sonsSoundpad.map(async ({ arquivo, ...som }) => ({ ...som, url: arquivo ? await enviarImagemCena(campanha.id, arquivo, "audio") : som.url, urlPersistida: arquivo ? undefined : som.urlPersistida })));
      await salvarSoundpadCampanha(campanha.id, sons); await recarregarCampanha();
    } catch (error) { setErro(error.message || "Nao foi possivel salvar o soundpad."); }
    finally { setSalvando(false); }
  };
  ...
  return <section className="campanha-dashboard">
    ...
    <nav className="campanha-conteudo-abas"><button className={abaCampanha === "cenas" ? "ativo" : ""} onClick={() => setAbaCampanha("cenas")}>Cenas e mapas</button><button className={abaCampanha === "musicas" ? "ativo" : ""} onClick={() => setAbaCampanha("musicas")}>Playlist</button><button className={abaCampanha === "soundpad" ? "ativo" : ""} onClick={() => setAbaCampanha("soundpad")}>Soundpad</button><button className={abaCampanha === "anotacoes" ? "ativo" : ""} onClick={() => setAbaCampanha("anotacoes")}>Anotacoes do mestre</button></nav>
    {abaCampanha === "cenas" ? <div className="campanha-pastas">{grupos.length ? grupos.map(([pasta, cenas]) => <section className={`campanha-pasta ${indicadorDrop?.tipo === "pasta" && indicadorDrop.id === pasta ? `drop-${indicadorDrop.posicao}` : ""}`} key={pasta} onDragOver={(event) => { if (arrastando?.tipo === "pasta") { event.preventDefault(); setIndicadorDrop({ tipo: "pasta", id: pasta, posicao: posicaoDrop(event) }); } }} onDrop={() => soltarPasta(pasta)}><button className="campanha-pasta-titulo" draggable onDragStart={() => setArrastando({ tipo: "pasta", id: pasta })} onDragEnd={() => { setArrastando(null); setIndicadorDrop(null); }} onClick={() => setPastasFechadas((estado) => ({ ...estado, [pasta]: !estado[pasta] }))}><Icon path={pastasFechadas[pasta] ? mdiChevronRight : mdiChevronDown} size={.72} /><Icon path={mdiFolderOutline} size={.78} /><strong>{pasta}</strong><span>{cenas.length}</span></button>{!pastasFechadas[pasta] && <div className="campanha-cenas">{cenas.map((cena) => <article key={cena.id} className={`${arrastando?.tipo === "cena" && arrastando.id === cena.id ? "arrastando" : ""} ${indicadorDrop?.tipo === "cena" && indicadorDrop.id === cena.id ? `drop-${indicadorDrop.posicao}` : ""}`} onDragOver={(event) => { if (arrastando?.tipo === "cena" && arrastando.pasta === (cena.pasta || "Sem pasta")) { event.preventDefault(); setIndicadorDrop({ tipo: "cena", id: cena.id, posicao: posicaoDrop(event) }); } }} onDrop={() => soltarCena(cena)}><button type="button" className="campanha-arrastar" draggable onDragStart={() => setArrastando({ tipo: "cena", id: cena.id, pasta: cena.pasta || "Sem pasta" })} onDragEnd={() => { setArrastando(null); setIndicadorDrop(null); }} title="Arrastar para ordenar"><Icon path={mdiDrag} size={.72} /></button><div className="campanha-cena-miniatura" style={{ backgroundImage: `url(${cena.imagensCena?.[0]?.url || cena.mapasBatalha?.[0]?.url || urlCena(cena, "cena") || urlCena(cena, "mapa")})` }}>{cena.mapasBatalha?.length > 0 && <span><Icon path={mdiMapOutline} size={.62} />{cena.mapasBatalha.length} mapa(s)</span>}</div><div className="campanha-cena-info"><strong>{cena.nome}</strong><p>{cena.descricao || "Sem descricao"}</p><small>{cena.imagensCena?.length || 0} cenas estaticas · {cena.mapasBatalha?.length || 0} mapas</small></div><div className="campanha-cena-acoes"><button onClick={() => abrirEditor(cena)} title="Editar"><Icon path={mdiPencilOutline} size={.72} /></button><button onClick={() => remover(cena)} title="Excluir"><Icon path={mdiDeleteOutline} size={.72} /></button></div></article>)}</div>}</section>) : <div className="campanha-sem-cenas"><Icon path={mdiImagePlusOutline} size={1.3} /><strong>Nenhuma cena salva</strong><button onClick={() => abrirEditor()}>Criar primeira cena</button></div>}</div> : abaCampanha === "musicas" ? <form className="campanha-playlist-editor" onSubmit={confirmarMusicas}><header><div><span>Trilha da campanha</span><h3>Playlist</h3><p>Adicione links individuais do YouTube e personalize como aparecem na mesa.</p></div><button type="button" onClick={adicionarMusica}><Icon path={mdiPlaylistMusic} size={.75} />Adicionar musica</button></header><div>{musicas.map((musica, indice) => <article key={musica.id} className={`${arrastando?.tipo === "musica" && arrastando.id === musica.id ? "arrastando" : ""} ${indicadorDrop?.tipo === "musica" && indicadorDrop.id === musica.id ? `drop-${indicadorDrop.posicao}` : ""}`} onDragOver={(event) => { if (arrastando?.tipo === "musica") { event.preventDefault(); setIndicadorDrop({ tipo: "musica", id: musica.id, posicao: posicaoDrop(event) }); } }} onDrop={() => soltarMusica(musica.id)}><button type="button" className="campanha-arrastar" draggable onDragStart={() => setArrastando({ tipo: "musica", id: musica.id })} onDragEnd={() => { setArrastando(null); setIndicadorDrop(null); }} title="Arrastar para ordenar"><Icon path={mdiDrag} size={.72} /></button><div className="campanha-musica-capa">{musica.capa ? <img src={musica.capa} alt="" /> : <Icon path={mdiPlaylistMusic} size={1.1} />}</div><div className="campanha-musica-campos"><label>Nome<input required value={musica.nome} onChange={(event) => atualizarMusica(musica.id, { nome: event.target.value })} /></label><label>URL da musica<input required type="url" placeholder="https://youtube.com/watch?v=..." value={musica.url} onChange={(event) => atualizarMusica(musica.id, { url: event.target.value })} /></label><label>URL da capa<input type="url" placeholder="Opcional: usa a miniatura do YouTube" value={musica.capa} onChange={(event) => atualizarMusica(musica.id, { capa: event.target.value })} /></label></div><span>{String(indice + 1).padStart(2, "0")}</span><button type="button" onClick={() => removerMusica(musica.id)} title="Remover musica"><Icon path={mdiDeleteOutline} size={.72} /></button></article>)}{!musicas.length && <div className="campanha-playlist-vazia"><Icon path={mdiPlaylistMusic} size={1.3} /><strong>Nenhuma musica adicionada</strong><button type="button" onClick={adicionarMusica}>Adicionar primeira musica</button></div>}</div><footer><span>{musicas.length} musica(s)</span><button type="submit" disabled={salvando}>{salvando ? "Salvando..." : "Salvar playlist"}</button></footer></form> : abaCampanha === "soundpad" ? <form className="campanha-soundpad-editor" onSubmit={confirmarSoundpad}><header><div><span>Mixer da campanha</span><h3>Soundpad</h3><p>Efeitos curtos e ambientes para disparar durante a sessao.</p></div><button type="button" onClick={adicionarSom}><Icon path={mdiPlus} size={.72} />Adicionar som</button></header><div className="campanha-soundpad-grade">{sonsSoundpad.map((som) => <article key={som.id}><header><span>{som.categoria || "Efeito"}</span><button type="button" onClick={() => removerSom(som.id)} title="Remover som"><Icon path={mdiDeleteOutline} size={.65} /></button></header><label>Nome<input required value={som.nome} onChange={(e) => atualizarSom(som.id, { nome: e.target.value })} /></label><label>Categoria<input value={som.categoria || ""} onChange={(e) => atualizarSom(som.id, { categoria: e.target.value })} placeholder="Ambiente, combate..." /></label><label>Arquivo de audio<input type="file" accept="audio/mpeg,audio/ogg,audio/wav,audio/webm,audio/mp4" onChange={(e) => selecionarAudioSom(som.id, e.target.files[0], e.target)} /><small>{som.arquivoNome || "Max. 20 MB"}</small></label><label>ou URL direta<input type="url" value={som.url || ""} onChange={(e) => atualizarSom(som.id, { url: e.target.value, arquivo: null })} placeholder="https://.../efeito.mp3" /></label><div className="soundpad-config-linha"><label>Volume <b>{som.volume ?? 70}%</b><input type="range" min="0" max="100" value={som.volume ?? 70} onChange={(e) => atualizarSom(som.id, { volume: Number(e.target.value) })} /></label><label>Fade in (s)<input type="number" min="0" max="20" step=".1" value={som.fadeIn ?? .4} onChange={(e) => atualizarSom(som.id, { fadeIn: Number(e.target.value) })} /></label><label>Fade out (s)<input type="number" min="0" max="20" step=".1" value={som.fadeOut ?? .7} onChange={(e) => atualizarSom(som.id, { fadeOut: Number(e.target.value) })} /></label></div><div className="soundpad-opcoes"><label><input type="checkbox" checked={Boolean(som.loop)} onChange={(e) => atualizarSom(som.id, { loop: e.target.checked })} />Loop contínuo</label><label><input type="checkbox" checked={som.sobrepor !== false} onChange={(e) => atualizarSom(som.id, { sobrepor: e.target.checked })} />Permitir sobreposição</label></div></article>)}{!sonsSoundpad.length && <div className="campanha-playlist-vazia"><Icon path={mdiPlaylistMusic} size={1.3} /><strong>Nenhum efeito cadastrado</strong><button type="button" onClick={adicionarSom}>Adicionar primeiro som</button></div>}</div><footer><span>{sonsSoundpad.length} som(ns)</span><button type="submit" disabled={salvando}>{salvando ? "Salvando..." : "Salvar soundpad"}</button></footer></form> : <AnotacoesCampanha campanhaId={campanha.id} />}
