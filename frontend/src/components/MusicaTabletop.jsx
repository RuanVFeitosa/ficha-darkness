...
import { mdiMusicNote, mdiPause, mdiPlay, mdiPlaylistMusic, mdiRepeatOnce, mdiSkipNext, mdiSkipPrevious, mdiVolumeHigh } from "@mdi/js";
...
const extrairVideoId = (valor) => {
  ...
  } catch { return String(valor || "").match(/(?:v=|youtu\.be\/|embed\/|shorts\/)([\w-]{11})/)?.[1] || ""; }
};
...
const buscarPlaylistDataApi = async (playlistId) => {
  ...
  do {
    ...
    const resposta = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?${parametros}`);
    const dados = await resposta.json();
    if (!resposta.ok) throw new Error(dados.error?.message || "A YouTube Data API recusou a playlist.");
    (dados.items || []).forEach((item) => {
      const videoId = item.snippet?.resourceId?.videoId;
      const titulo = item.snippet?.title;
      if (!videoId || ["Private video", "Deleted video"].includes(titulo)) return;
      faixas.push({
        videoId,
...
const MusicaTabletop = ({ campanhaId, musicasCampanha = null, estadoRemoto = null, aoAlterarEstado = null, controlavel = true }) => {
  const musicasChave = Array.isArray(musicasCampanha)
    ? musicasCampanha.map((musica) => `${musica.id}:${musica.url}:${musica.nome}:${musica.capa}`).join("|")
    : "playlist-legada";
  ...
  const [volume, setVolume] = useState(() => {
    const salvo = Number(localStorage.getItem(`darkness_volume_mesa_${campanhaId}`));
  ...
  const estadoRef = useRef({ indice: 0, tocando: false, volume: 45, repetindo: false });
  ...
  const publicarEstado = (mudancas = {}) => {
    ...
    aoAlterarEstado({
      indice: Math.max(0, playerRef.current?.getPlaylistIndex?.() ?? atual.indice),
  ...
  useEffect(() => {
    if (controlavel || !playerPronto || !estadoRemoto || !playerRef.current) return;
    const player = playerRef.current;
    const indice = Math.max(0, Number(estadoRemoto.indice) || 0);
    const decorrido = estadoRemoto.tocando && estadoRemoto.atualizadoEm
      ? Math.max(0, (Date.now() - new Date(estadoRemoto.atualizadoEm).getTime()) / 1000)
      : 0;
    ...
    if (estadoRemoto.tocando) player.playVideo?.(); else player.pauseVideo?.();
  }, [controlavel, estadoRemoto, playerPronto]);

  useEffect(() => {
    const modoCampanha = Array.isArray(musicasCampanha);
    if ((!playlistId && !modoCampanha) || !elementoRef.current) return undefined;
    ...
    Promise.all([
      carregarApiYoutube(),
      (modoCampanha ? Promise.resolve(musicasCampanha.map((musica, indice) => {
        const videoId = extrairVideoId(musica.url);
        return videoId ? { videoId, titulo: musica.nome || `Musica ${indice + 1}`, miniatura: musica.capa || `https://i.ytimg.com/vi/${videoId}/mqdefault.jpg` } : null;
      }).filter(Boolean)) : buscarPlaylistDataApi(playlistId)).catch((error) => {
        setErro(`Data API: ${error.message} Tentando o carregamento direto do YouTube.`);
        return null;
      }),
    ]).then(([YT, faixasApi]) => {
      if (cancelado) return;
      ...
      if (idsApi.length) {
        ...
        setErro("Nenhuma URL de video valida foi encontrada na playlist da campanha.");
      }
      ...
      playerRef.current = new YT.Player(alvo, {
        ...
        events: {
          ...
          onStateChange: (event) => {
            const indice = Math.max(0, event.target.getPlaylistIndex?.() || 0);
            setTocando(event.data === YT.PlayerState.PLAYING);
            setIndiceAtual(indice);
            if (event.data === YT.PlayerState.ENDED && repetindoRef.current) event.target.playVideoAt(indice);
            if (controlavel && [YT.PlayerState.PLAYING, YT.PlayerState.PAUSED, YT.PlayerState.ENDED].includes(event.data)) publicarEstado({ indice, tocando: event.data === YT.PlayerState.PLAYING, tempo: event.data === YT.PlayerState.ENDED ? 0 : Number(event.target.getCurrentTime?.() || 0) });
  ...
  const tocarFaixa = (indice) => { if (!controlavel) return; playerRef.current?.playVideoAt?.(indice); setIndiceAtual(indice); publicarEstado({ indice, tempo: 0, tocando: true }); };
  const alternarReproducao = () => {
    const vaiTocar = !tocando;
    if (vaiTocar) playerRef.current?.playVideo?.(); else playerRef.current?.pauseVideo?.();
    publicarEstado({ tocando: vaiTocar });
  };

  return <section className={`musica-tabletop ${controlavel ? "" : "musica-ouvinte"}`}>
    {!Array.isArray(musicasCampanha) && <form className="musica-vinculo" onSubmit={carregarPlaylist}><label><Icon path={mdiPlaylistMusic} size={.72} /><input value={link} onChange={(event) => setLink(event.target.value)} placeholder="Cole o link da playlist do YouTube" /></label><button type="submit">Carregar</button></form>}
    {erro && <p className="musica-erro">{erro}</p>}
    <div className="musica-conteudo"><section className="musica-lista"><header><span>Musicas da playlist</span><b>{faixas.length || "..."}</b></header><div>{faixas.length ? faixas.map((faixa, indice) => <button key={`${faixa.videoId}-${indice}`} className={indice === indiceAtual ? "ativo" : ""} onClick={() => tocarFaixa(indice)}><img src={faixa.miniatura} alt="" /><span><small>{String(indice + 1).padStart(2, "0")}</small><strong>{faixa.titulo}</strong></span>{indice === indiceAtual && <Icon path={tocando ? mdiVolumeHigh : mdiMusicNote} size={.62} />}</button>) : <p>{Array.isArray(musicasCampanha) ? "Adicione musicas no dashboard da campanha." : playlistId ? "Carregando musicas..." : "Adicione uma playlist para comecar."}</p>}</div></section>
      <section className="musica-player">{!controlavel && <img className="musica-capa-ouvinte" src={faixas[indiceAtual]?.miniatura} alt="" />}<header><div><span>{controlavel ? "Tocando agora" : "Trilha da campanha"}</span><strong>{faixas[indiceAtual]?.titulo || "Nenhuma musica selecionada"}</strong></div><div><button disabled={!controlavel} onClick={() => { playerRef.current?.previousVideo?.(); publicarEstado({ indice: Math.max(0, indiceAtual - 1), tempo: 0, tocando: true }); }} title="Anterior"><Icon path={mdiSkipPrevious} size={.75} /></button><button className="principal" onClick={controlavel ? alternarReproducao : () => estadoRemoto?.tocando ? playerRef.current?.playVideo?.() : playerRef.current?.pauseVideo?.()} title={controlavel ? (tocando ? "Pausar" : "Tocar") : "Ativar audio sincronizado"}><Icon path={tocando ? mdiPause : mdiPlay} size={.78} /></button><button disabled={!controlavel} onClick={() => { playerRef.current?.nextVideo?.(); publicarEstado({ indice: Math.min(faixas.length - 1, indiceAtual + 1), tempo: 0, tocando: true }); }} title="Proxima"><Icon path={mdiSkipNext} size={.75} /></button><button disabled={!controlavel} className={repetindo ? "ativo" : ""} onClick={() => { const valor = !repetindo; setRepetindo(valor); publicarEstado({ repetindo: valor }); }} title={repetindo ? "Desativar repeticao" : "Repetir musica"}><Icon path={mdiRepeatOnce} size={.72} /></button></div></header><div className="musica-youtube-player" ref={elementoRef} /><label className="musica-volume"><Icon path={mdiVolumeHigh} size={.65} /><input type="range" min="0" max="100" value={volume} disabled={!controlavel} onChange={(event) => { const valor = Number(event.target.value); setVolume(valor); playerRef.current?.setVolume?.(valor); publicarEstado({ volume: valor }); }} /><span>{volume}%</span></label></section>
      {!controlavel && <label className="musica-volume-local" title="Meu volume"><Icon path={mdiVolumeHigh} size={.55} /><input type="range" min="0" max="100" value={volume} onChange={(event) => { const valor = Number(event.target.value); setVolume(valor); playerRef.current?.setVolume?.(valor); localStorage.setItem(`darkness_volume_mesa_${campanhaId}`, String(valor)); }} /></label>}
    </div>
  </section>;
};
